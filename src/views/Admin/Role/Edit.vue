...
export default {
    ...
    created() {
        ...
        this.$axios('role/' + this.$route.params.id).then(response => {
            ...
            this.rolePermissions = permissions.map(function(p) {
                return {
                    id: p.id,
                    tag:
                        p.pivot.limitations && p.pivot.limitations
                            ? p.pivot.limitations.tag
                            : null,
                    field_selection:
                        p.pivot.limitations && p.pivot.limitations
                            ? p.pivot.limitations.field_selection
                            : null
                };
            });
            ...
            this.role.menu_items = menu_items
                ? menu_items.map(p => p.id)
                : null;
    ...
    methods: {
        updateLimitationAndField({ permission_id, fields, limitations }) {
            const except_fields = this.fields.filter(
                r => r.permission_id !== permission_id
            );
            this.fields = except_fields.concat(fields);

            let except_limitation = this.rolePermissions.filter(
                r => r.id !== permission_id
            );
