      ...
      <v-list
        ...
        <template  v-for="item in menuItems" >
          <v-list-tile :key="`${item.id}`">
            ...
            <v-list-tile-content>
              <v-list-tile-title @click.stop="switchPermission(item)">{{item.title}}</v-list-tile-title>
              <div v-if="isDeveloper">
              <v-list-tile-sub-title v-for="permission in item.permissions" 
                :key="`item${permission.id}`">
                  {{permission.name}} <v-icon small @click.stop="showLimitation(permission)">settings</v-icon>
                  <router-link :to="`/permission/${permission.id}/edit`">
...
export default {
  ...
  computed: {
    roleMenu () {
      ...
      return this.menus.filter(m => {
         return m.role_ids && m.role_ids.includes(parseInt(this.$route.params.id))
      })
    }
  },
  created () {
  ...
  methods: {
    ...
    switchPermission(item) {
      let menu_items = this.$helper.getProp(this.lqForm.formValues, 'menu_items', []);
      ...
      this.$lqForm.setElementVal(this.lqForm.name, 'menu_items', menu_items)
    },
    changeMenu (item) {
      let items = []
