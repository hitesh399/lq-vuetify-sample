        ...
        <VueNestable v-if="!readOnly" v-model="menuNestedItems" @change="whenChange">
            <template slot-scope="{ item }">
                <span class="row-in-line">
                    <VueNestableHandle :item="item" style="display:inline; margin-right: 10px">
                        <v-icon>menu</v-icon>
                    </VueNestableHandle>
            ...
            <!-- <VueNestableHandle slot-scope="{ item }" :key="`${item.id}`" :item="item">
                <div>
                    <v-icon>menu</v-icon>
                    <span class="row-in-line">
                        <span>{{ item.title }} ({{item.menu_order}})</span>
                        <router-link
                            :to="{name: 'application_menu_item.edit', params: {id: item.id}}"
                        >
                            <v-icon small>edit</v-icon>
                        </router-link>
                        <v-icon small @click.stop="deleteMenuItem(item)">delete</v-icon>
...
export default {
    ...
    created() {
        this.$axios(
            'application-menu-item?page_size=-1&application_menu_id=' +
                this.menuId +
                '&role_id' +
                this.roleId
        ).then(response => {
    ...
    methods: {
        ...
        updateOrder() {
            ...
            this.$axios
                .put('application-menu-item/re-arrange/order', {
                    data
                })
                .then(response => {});
        },
        deleteMenuItem(item) {
            this.confirmAndAction('application-menu-item/' + item.id, {}).then(
                response => {
                    let index = null;
